...
class MoveDepositToOutputDirSpec extends UnitSpec with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    ...
    if (settings.outputDepositDir.exists)
      settings.outputDepositDir.delete()
    settings.outputDepositDir.createIfNotExists(asDirectory = true, createParents = true)
    settings.outputDepositDir.toJava should exist
  }
